...
<template>
  <div class="user-account-menu">
    ...
    <div v-if="open" class="account-dropdown" role="menu">
      ...
      <button class="account-dropdown__manage" type="button" role="menuitem" @click="openAccountManagement">
        {{ user.currentUser?.role === 'ADMIN' ? '后台管理' : '修改资料' }}
      </button>
      <button class="account-dropdown__logout" type="button" role="menuitem" :disabled="loggingOut" @click="logout">{{ loggingOut ? '正在退出…' : '退出系统' }}</button>
    </div>
  </div>
</template>
...
.account-dropdown { position: absolute; top: calc(100% + 8px); right: 0; width: 158px; padding: 7px; background: #042941f7; border: 1px solid #177fa9; border-radius: 5px; box-shadow: 0 8px 22px #001524aa; }.account-dropdown p { margin: 2px 7px 7px; overflow: hidden; color: #74a6ba; font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }.account-dropdown button { width: 100%; min-height: 34px; padding: 0 8px; color: #bfeeff; background: transparent; border: 1px solid #16658a; border-radius: 3px; font-size: 13px; text-align: left; cursor: pointer; }.account-dropdown button + button { margin-top: 6px; }.account-dropdown button:hover { color: #fff; background: #0d729c; }.account-dropdown .account-dropdown__logout { color: #f5c4c9; border-color: #7c3744; }.account-dropdown .account-dropdown__logout:hover { background: #a13b4c; }.account-dropdown button:disabled { cursor: wait; opacity: .7; }
</style>
