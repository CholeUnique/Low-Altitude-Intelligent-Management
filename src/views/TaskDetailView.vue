    ...
    <div v-if="task && organization" class="detail-layout">
      ...
      <main class="detail-main">
        ...
        <section class="detail-panel process-panel">
          ...
          <div class="process-flow"><div v-for="(node,index) in task.workflow" :key="node.key" class="process-step" :class="node.status"><i>{{ node.status === 'pending' ? index + 1 : '✓' }}</i><b>{{ node.name }}</b><small>{{ node.time || '待开始' }}</small></div></div>
        </section>
