    ...
    <div v-if="task && organization" class="detail-layout">
      ...
      <main class="detail-main">
        ...
        <section class="detail-panel process-panel">
          ...
          <div class="process-flow" :style="{ gridTemplateColumns: `repeat(${task.workflow.length}, minmax(0, 1fr))` }"><div v-for="(node,index) in task.workflow" :key="node.key" class="process-step" :class="node.status"><i>{{ node.status === 'pending' ? index + 1 : '✓' }}</i><b :title="node.name">{{ node.name }}</b><small>{{ node.time || '待开始' }}</small></div></div>
        </section>
...
.detail-layout { height: calc(100vh - 70px); display: grid; grid-template-columns: 240px 1fr; gap: 7px; }.detail-left { display: grid; grid-template-rows: 1fr 190px; gap: 7px; min-height: 0; }.detail-main { min-width: 0; display: grid; grid-template-rows: 76px minmax(205px,1fr) 152px minmax(180px,.85fr); gap: 7px; }
...
.related-resources button { width: 100%; padding: 8px 10px; color: #87adbc; background: transparent; border: 0; border-bottom: 1px solid #0a3b56; text-align: left; font-size: 14px; cursor: pointer; }
...
.map-info-grid { min-height: 0; display: grid; grid-template-columns: 2.15fr 1fr; gap: 7px; }.task-map-panel { display: grid; grid-template-rows: 34px 1fr; }.area-info .detail-title { font-size: 18px; }.area-info dl { display: grid; grid-template-columns: 92px 1fr; margin: 7px 12px; }.area-info dt,.area-info dd { margin: 0; padding: 7px 0; border-bottom: 1px solid #0a3c57; font-size: 17px; }.area-info dt { color: #769eaf; }.area-info dd { color: #d1edf4; }.photo-strip { display: grid; grid-template-columns: repeat(3,1fr); gap: 4px; margin: 8px; }.photo-strip i { height: 45px; background: linear-gradient(145deg,#547963,#173c49); border: 1px solid #155f7d; }
.process-panel { display: grid; grid-template-rows: 34px 1fr; }.process-flow { position: relative; display: grid; align-items: start; padding: 12px 26px 9px; }.process-step { position: relative; min-width: 0; display: grid; grid-template-rows: 25px minmax(30px,auto) 16px; justify-items: center; gap: 5px; color: #6f94a8; text-align: center; }.process-step:not(:last-child)::after { content: ""; position: absolute; z-index: 0; top: 11px; left: calc(50% + 12px); width: calc(100% - 24px); height: 2px; background: #315a73; }.process-step.done::after { background: #35d7b0; box-shadow: 0 0 6px #35d7b077; }.process-step.active::after { background: linear-gradient(90deg, #1dcdb0, #209aff); }.process-step i { position: relative; z-index: 1; width: 22px; height: 22px; display: grid; place-items: center; border: 2px solid #547a91; border-radius: 50%; color: #8daabc; background: #04243e; font-size: 12px; font-style: normal; line-height: 1; }.process-step b { display: grid; align-items: center; min-height: 30px; padding: 0 4px; color: inherit; font-size: 14px; line-height: 1.25; }.process-step small { color: #5f8496; font-size: 12px; line-height: 1.15; white-space: nowrap; }.process-step.done { color: #5ce8c0; }.process-step.done i { border-color: #25d8af; color: white; background: #20cfa7; box-shadow: 0 0 8px #20cfa788; }.process-step.active { color: #70caff; }.process-step.active i { border-color: #74d6ff; color: white; background: #168ff0; box-shadow: 0 0 9px #168ff0; }
.detail-bottom { min-height: 0; display: grid; grid-template-columns: 2.2fr .85fr; gap: 7px; }.record-tabs { height: 42px; display: flex; border-bottom: 1px solid #0b4c6d; }.record-tabs button { padding: 0 17px; color: #91b9c9; background: transparent; border: 0; font-size: 17px; }.record-tabs button.active { color: #78e8f5; border-bottom: 2px solid #18c5df; }.record-tabs span { flex: 1; }.records-panel table { width: 100%; border-collapse: collapse; font-size: 14px; }.records-panel th,.records-panel td { padding: 9px 8px; border-bottom: 1px solid #0a3b55; text-align: left; }.records-panel th { color: #c4e2ed; background: #063452; font-size: 15px; }.records-panel td { color: #c3dfe8; }.records-panel em { color: #55ddb5; font-style: normal; }.records-panel a { color: #57d2f7; }
