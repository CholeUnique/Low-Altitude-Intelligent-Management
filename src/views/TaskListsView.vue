  ...
  <CockpitPageLayout center-subtitle variant="repository" title="低空治理任务总览" :subtitle="`${user.organization.name} · ${sceneFilter ? getScene(user.organization, sceneFilter)?.name : '全部场景'}`">
    ...
    <section ref="taskTable" class="task-table">
      ...
      <div v-for="task in pagedTasks" :key="task.id" class="task-table-row">
        ...
        <span class="workflow-mini" :style="{ gridTemplateColumns: `repeat(${task.workflow.length}, minmax(0, 1fr))` }" :aria-label="`任务进度 ${task.progress}%`"><span v-for="node in task.workflow" :key="node.key" class="workflow-step" :class="node.status"><i>{{ node.status === 'pending' ? '' : '✓' }}</i><small :title="node.name">{{ node.name }}</small></span></span>
