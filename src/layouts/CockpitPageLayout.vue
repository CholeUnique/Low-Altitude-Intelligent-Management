...
<template>
  <div class="cockpit-page" :class="{ 'cockpit-page--repository': variant === 'repository', 'cockpit-page--center-subtitle': centerSubtitle }">
    <PrimaryHeader />
    <div v-if="$slots.action" class="cockpit-page-actions"><slot name="action" /></div>
    <main class="cockpit-page-body"><slot /></main>
  </div>
</template>

<style scoped lang="scss">
.cockpit-page { width: 100%; min-width: 0; min-height: 100dvh; height: 100dvh; display: flex; flex-direction: column; color: #d6f4ff; background: radial-gradient(circle at 50% -15%, #0a4265, transparent 38%), linear-gradient(#031a31, #010b18); }
.cockpit-page-header { position: relative; height: 54px; flex: 0 0 54px; display: flex; align-items: center; gap: 10px; padding: 0 10px; background: linear-gradient(180deg, #06385a, #031d37); border-bottom: 1px solid #0b648e; box-shadow: 0 3px 14px #0009; }
.back-button { padding: 5px 13px; color: #8dc5d8; background: #062a47; border: 1px solid #15597a; cursor: pointer; }.page-logo { color: #38e3f4; font-size: 20px; text-shadow: 0 0 8px #27d9eb; }.page-title { min-width: 0; }.page-title h1 { display: inline; margin: 0; font-size: 20px; letter-spacing: 2px; white-space: nowrap; }.page-title small { position: absolute; top: 50%; left: 50%; margin: 0; color: #588398; font-size: 14px; transform: translate(-50%, -50%); white-space: nowrap; }.header-spacer { flex: 1; min-width: 0; }.system-state { display: flex; align-items: center; gap: 7px; color: #55eac3; font-size: 14px; white-space: nowrap; }.system-state i { width: 9px; height: 9px; border-radius: 50%; background: #25e0b0; box-shadow: 0 0 8px #25e0b0; }.cockpit-page-header time,.header-user { color: #739eb0; font-size: 14px; white-space: nowrap; }.cockpit-page-body { flex: 1; min-height: 0; overflow: auto; padding: 8px; }
.cockpit-page:not(.cockpit-page--repository) .page-title small { display: none; }
.cockpit-page:not(.cockpit-page--repository) .system-state,.cockpit-page:not(.cockpit-page--repository) .cockpit-page-header time,.cockpit-page:not(.cockpit-page--repository) .header-user { font-size: 13px; }
...
.cockpit-page-actions { position:absolute; z-index:5; top:70px; right:12px; }
@media (max-width: 1100px) { .cockpit-page-header { gap: 7px; padding-inline: 7px; }.page-title h1 { font-size: 17px; }.cockpit-page-header time { display: none; }.system-state { font-size: 12px; }.back-button { padding-inline: 9px; } }
</style>
