  ...
  <div class="global-live-cruise" :class="{ 'is-fullscreen': props.fullscreen }">
    ...
    <section v-if="props.fullscreen" class="fullscreen-live">
      ...
      <article v-else class="fullscreen-screen">
        ...
        <div v-else class="demo-live demo-live--fullscreen"><i class="target-reticle"></i><span class="live-badge">● LIVE</span><div class="telemetry">ALT 126.4m　SPD 8.2m/s　HDG 087°</div></div>
    ...
    <section v-if="!props.fullscreen" class="video-wall-panel overview-panel">
      ...
      <div v-else class="video-wall">
        <article v-for="stream in streams" :key="stream.id" class="video-screen" :class="{ active: activeDeviceId === stream.id }" @click="activeDeviceId = stream.id">
          ...
          <div v-else class="video-placeholder demo-live"><i class="target-reticle"></i><span class="live-badge">● LIVE · {{ stream.aircraftId || '未命名设备' }}</span><small class="telemetry">ALT {{ 92 + streams.indexOf(stream) * 17 }}m　SPD {{ 6 + streams.indexOf(stream) }}.{{ streams.indexOf(stream) + 2 }}m/s</small></div>
...
/* 无真实流时仍以演示图传呈现完整直播界面，便于效果图评审。 */
.video-placeholder.demo-live { position:relative; display:block; overflow:hidden; color:#e9f9ff; background-image:linear-gradient(#00172126,#00172159),linear-gradient(37deg,transparent 47%,#c8b28a 48% 53%,transparent 54%),repeating-linear-gradient(90deg,#4f793e 0 9%,#698949 9% 18%,#9b994a 18% 27%,#436b37 27% 36%); }
.video-screen:nth-child(2n) .demo-live { background-image:linear-gradient(#00172120,#00172166),linear-gradient(118deg,transparent 40%,#657279 41% 51%,transparent 52%),repeating-linear-gradient(25deg,#335e3b 0 12%,#52764a 12% 23%,#7d8d53 23% 34%); }
.video-screen:nth-child(3n) .demo-live { background-image:linear-gradient(#0017211f,#00172155),radial-gradient(ellipse at 64% 46%,#a37b4a 0 17%,transparent 18%),repeating-linear-gradient(145deg,#285838 0 11%,#3f7045 11% 22%,#608044 22% 34%); }
.demo-live::before,.demo-live::after { content:""; position:absolute; pointer-events:none; }.demo-live::before { inset:8px; border:1px solid #d8f8ff55; background:linear-gradient(90deg,transparent 49.7%,#d8f8ff55 50%,transparent 50.3%),linear-gradient(transparent 49.7%,#d8f8ff55 50%,transparent 50.3%); }.demo-live::after { inset:0; background:repeating-linear-gradient(0deg,#fff0 0 3px,#c6efff0b 4px); }
.target-reticle { position:absolute; z-index:2; left:50%; top:50%; width:42px; height:42px; border:1px solid #ffef84; border-radius:50%; transform:translate(-50%,-50%); box-shadow:0 0 7px #ffd85a88; }.target-reticle::before,.target-reticle::after { content:""; position:absolute; background:#ffef84; }.target-reticle::before { left:-10px; right:-10px; top:50%; height:1px; }.target-reticle::after { top:-10px; bottom:-10px; left:50%; width:1px; }
.live-badge { position:absolute; z-index:3; top:9px; left:10px; padding:3px 6px; color:#fff!important; background:#ce2939cc; border-radius:2px; font-size:10px!important; }.telemetry { position:absolute; z-index:3; right:9px; top:10px; color:#e8fbff!important; text-shadow:0 1px 2px #001827; font:10px monospace!important; }.demo-live--fullscreen { width:100%; height:100%; }
</style>
